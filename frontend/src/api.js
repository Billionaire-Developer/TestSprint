...
export const api = {
  ...
  getAdminSchoolsSummary: () => request("/admin/schools-summary"),
  ...
  getNoteSubjects: () => request("/notes/subjects"),
  getNotesForSubject: (subject) => request(`/notes/${subject}`),
  getNote: (subject, noteId) => request(`/notes/${subject}/${noteId}`),
  ...
  adminCreateNote: (subject, className, title, content) =>
    request("/admin/notes", {
      method: "POST",
      body: JSON.stringify({ subject, className, title, content })
    }),
  ...
  adminDeleteNote: (id) => request(`/admin/notes/${id}`, { method: "DELETE" }),
